...
import {
  ...
  setMetricParams,
  updateColumnConfig,
} from "@/reducers/indicatorAdvance";
...
export default function AdvanceCodeMode() {
  ...
  const codeMode = useSelector(
    (state: StoreState) => state.indicatorAdvance.codeMode
  );
  ...
  useEffect(() => {
    const contents = map(
      code,
      (item, index) =>
        `<span class="${item.type}" data-index="${item.key}" contenteditable="${false}">${item.type === "symbol" ? inidcatorSymbolMap[item.name] : item.name}</span>`
    )
      .join("")
      .trim();
  ...
  const parseSource2GraphData = (arr: any[]) => {
    ...
    arr.forEach((item, index: number) => {
      ...
      if (index) {
        ...
        const prevId = isArray(prevSibling)
          ? findLast(prevSibling, { type: "symbol" })?.id
          : prevSibling.id;
        if (item.type === "symbol") {
          const edge = find(edges, { source: prevId });
          const source = edge ? edge.target : prevId;
          const edg: { source: string; target: string; end?: boolean } = {
            source: source,
            target: item.id,
          };
          ...
          const id = isArray(item)
            ? findLast(item, { type: "symbol" })?.id
            : item.id;
  ...
  const handleOk = () => {
    ...
    if (nodes.length) {
      ...
      const lastSymbol =
        findLast(arr, { type: "symbol" }) ||
        findLast(nodes, { type: "symbol" });
      if (lastSymbol) {
