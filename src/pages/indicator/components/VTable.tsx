...
import { setLoading, setTableData, updateDisabledField, setFuncResult, updateSelectedColumns, updateExtraColumns } from "@/reducers/indicator";
...
export default function VTable(props: {width: number, height: number}) {
  ...
  useEffect(() => {
    func && getFuncResult({dimension: getDimensionObj(dimension), func, groupBy: getGroupByObj(compact(groupBy)), extraColumns, query}, function(success: boolean, response: any) {
      if (success) {
